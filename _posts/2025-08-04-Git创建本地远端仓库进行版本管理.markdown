...
- 本地创建bare仓库
  - 进入你的裸仓库目录, 比如`F:/GitBareRepos/docs`, 开启`git bash`命令行
  - 输入`git init --bare`, 回车

- 初始化仓库
  - 进入自己想要跟踪的目录, 比如`_docs`
  - 输入`git init`, 回车
  - 输入`git remote add origin F:/GitBareRepos/docs`, 请把`F:/GitBareRepos/docs`换成自己的路径, 回车
    - 直接在Windows中拷贝路径时, 分隔符需要替换为`/`
    - SourceTree中, 路径分隔符为Windows默认分割符即可.
  - 初次推送, 输入`git push origin main`

- 单独将裸仓库指定为某个仓库的远端

  ```bash
  git remote add origin F:/GitBareRepos/docs
  ```
...
先进入你要存放仓库的目录, 然后进入`git bash`, 执行拉取命令即可, 千万注意, windows下的反斜杠使用方法, 直接复制路径, 反斜杠是不对的.

```bash
git clone E:/GitBareRepos/YourRepo.git
```
