...
顾名思义, 这款分析工具仅限定搭载Snapdragon系列SoC的手机. 该Profiler可以截帧. 仅获取CPU/GPU数据时apk包无需<font color=orange>debuggable</font>, 但如果需要截帧, 还是需要设置<font color=orange>debuggable</font>. 优点是截帧后可以看到大部分渲染流程的性能参数. 缺点是, 卡/慢/容易奔溃. 
